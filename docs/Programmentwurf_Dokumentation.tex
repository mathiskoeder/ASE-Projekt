...
einen Rechner, geben ihre Züge nacheinander in algebraischer Notation ein (\texttt{e2-e4},
\texttt{e7-e8=Q}, \texttt{O-O}) und sehen das Brett zwischen den Zügen als Unicode-ASCII-Ausgabe.

Implementiert sind alle regulären Schachregeln einschließlich Rochade, En passant,
Bauernumwandlung sowie der Endspielerkennung Schachmatt, Patt, 50-Züge-Regel und dreifacher
Stellungswiederholung. Partien können in einer einfachen, von Hand lesbaren Datei gespeichert und
später wieder geladen werden (Format: FEN-Stellung plus PGN-light-Zugfolge).
...
Clean Architecture nach Robert C. Martin organisiert ein System in konzentrische Schichten, deren
Abhängigkeiten ausschließlich nach innen gerichtet sind. Innerhalb dieser Schale liegen die
Domänen-Entitäten und Geschäftsregeln, in den äußeren Schichten Anwendungs-Use-Cases und
schließlich Adapter wie UI, Datenbank oder Frameworks. Die zentrale Regel — die \emph{Dependency
...
Domain nach sich. Heute trägt der \texttt{MoveService} (Application) diese Verantwortung —
...
\texttt{Board} und \texttt{MoveHistory}), die Domain kennt aber nicht die Datei — sie sieht nur
das Interface.
...
konkreten Implementierung. Im Konstruktor wird das Repository injiziert; ob dahinter
\texttt{InMemoryGameRepository}, \texttt{FileGameRepository} oder im Test ein
\texttt{Mockito.mock(GameRepository.class)} steht, ist transparent.
...
\texttt{Game} ist die Aggregat-Wurzel. Es schließt \texttt{Board}, \texttt{MoveHistory},
\texttt{Player}, \texttt{GameStatus} und die ziehende Farbe als Konsistenzgrenze ein. Alle
regelrelevanten Mutationen (\texttt{makeMove}, \texttt{resign}) laufen über \texttt{Game}, sodass
Invarianten wie \glqq{}Status-Wechsel nur nach gültigem Zug\grqq{} oder \glqq{}aktive Farbe
wechselt nach jedem Zug\grqq{} an einer Stelle gewahrt bleiben. Externe Klassen erhalten das Brett
...
\textbf{Refactoring 2: Extract Class --- \texttt{MoveValidator} und \texttt{CheckDetector}
